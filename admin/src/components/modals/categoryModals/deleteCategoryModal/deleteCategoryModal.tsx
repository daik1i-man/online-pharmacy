...
export default function DeleteCategoryModal() {
    ...
    if (!context) {
        throw new Error('SomeComponent must be used within an DeleteCategory')
    }

    const { openDeleteCategoryModal, setOpenDeleteCategoryModal, deletecurrentCategory } = context;

    async function DeleteCategoryHandler() {
        setLoading(true)
        try {
            await axios.post('http://localhost:5000/admin-controll/categories/delete', {
                id: deletecurrentCategory?.id
            });
