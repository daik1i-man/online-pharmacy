...
export default function TakeOrderModal() {
    ...
    if (!context) {
        throw new Error('SomeComponent must be used within an Take The order context')
    }

    const { openTakeOrderModal, setOpenTakeOrderModal, takeCurrentOrder } = context
    ...
    const takeOrderHandler = async () => {
        setLoading(true)
        try {
            await axios.post('http://localhost:5000/admin-controll/orders/delete', {
                id: takeCurrentOrder
            }).then(() => setOpenTakeOrderModal(false))
        } catch (error) {
            console.log(error);
        }
        setLoading(false)
