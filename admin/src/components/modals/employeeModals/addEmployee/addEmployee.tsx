...
export default function AddEmployeeModal() {
    ...
    if (!context) {
        throw new Error('SomeComponent must be used within an AddEmployee')
    }

    const { openAddEmployeeModal, setOpenAddEmployeeModal } = context
    ...
    const handleSubmit = async () => {
        setLoading(true)
        try {
            await axios.post('http://localhost:5000/admin-controll/employees/add', {
