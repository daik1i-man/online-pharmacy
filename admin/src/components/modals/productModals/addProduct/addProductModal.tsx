...
import { fetchAllcategories } from './fetAllCategories';
import { dataProps, PriceProps, NameProps, QunatityProps, DescriptionProps } from './types';
import { uploadFile } from './fetAllCategories';
...
export default function AddProductModal() {
    ...
    const initialNameState: NameProps = {
        name: ''
    }

    const priceInitialState: PriceProps = {
        price: ''
    }

    const initialQuantityState: QunatityProps = {
        quantity: ''
    }

    const initialDescriptionState: DescriptionProps = {
        description: ''
    }

    const [price, setPrice] = useState(priceInitialState)
    const [name, setName] = useState(initialNameState)
    const [productQuantity, setProductQuantity] = useState(initialQuantityState)
    const [description, setDescription] = useState(initialDescriptionState)


    if (!context) {
        throw new Error('SomeComponent must be used within an AddProduct')
    }

    const { openAddProductModal, setOpenAddProductModal } = context
    ...
    const priceChangeHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setPrice({ price: formattedNumber });
    }
    ...
    const descrptionChangeHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const addProductHandler = async () => {
        setLoading(true)
        try {
            if (file && price.price && name.name && description.description && productQuantity.quantity && selectedCategory && file) {
                const url = await uploadFile(file)

                await axios.post('http://localhost:5000/admin-controll/products/add', {
                    img_url: url,
    ...
    return (
        <Dialog className="relative z-50" open={openAddProductModal} onClose={closeHandler}>
            <DialogBackdrop
                transition
                ...
                <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    ...
                    >
                        ...
                        <div className="sm:mx-auto sm:w-full sm:max-w-xl">
                            <form className="space-y-6" method="POST" onSubmit={addProductHandler}>
                                <div className="w-full my-3 ustify-center">
                                    <p className='block text-sm font-medium leading-6 text-gray-900 my-3'>Add image</p>
                                    {file === undefined ? (
                                        <Label
                                ...
                                <div className="flex items-center max-w-xl justify-between mx-auto">
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-2">
                                            <div className="flex items-center border-0 ring-1 ring-gray-300 rounded-md p-1 px-4 border-gray-900">
                                                <input
                                                    id="price"
                                                    name="price"
                                                    type="text"
                                ...
                                <div className="">
                                    <Textarea
                                        ...
                                        onChange={(e) => descrptionChangeHandler(e)}
                                        classNames={{
                                            base: "max-w-xl",
