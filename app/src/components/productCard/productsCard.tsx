...
export default function ProductsCard({ name, id, img_url, price, cart, favourites }: productCardDatasProps) {
    ...
    return (
        <Card
            className="relative shadow-none w-[230px] p-2 pb-4 mx-auto"
        >
            ...
            <div className="flex items-center justify-between">
                <span className="text-gray-900 text-end">{`${price} UZS`}</span>
                {cart ? (
                    <button
                        onClick={() => onDelete(id)}
                        className="relative p-2 text-xs font-medium text-center text-red-500 bg-gray-200 rounded-full"
                    >
                    ...
                    <button
                        onClick={onAdd}
                        className="relative p-2 text-xs font-medium text-center text-gray-600 bg-gray-200 rounded-full"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
